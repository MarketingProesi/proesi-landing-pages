...
function createCard(name, folder, link, categoryId) {
  ...
  img1.loading = "lazy";
  img1.decoding = "async";
  img1.addEventListener("load", scheduleHeightUpdate);
  ...
  img2.addEventListener("load", scheduleHeightUpdate);
...
function renderCategory(id) {
  ...
  cat.products.forEach(([name, folder, link]) => {
    frag.appendChild(createCard(name, folder, link, cat.id));
  });
...
function setView(view) {
  ...
  if (view === "todos") {
    ...
    renderCategory(currentCategory || CATEGORIES[0].id);
  }
  scheduleHeightUpdate();
}
...
function sendHeight() {
  // Measure the actual content height. We deliberately avoid
  // documentElement.scrollHeight because it is clamped to the current
  // viewport (iframe) height — once the parent grows the iframe, the html
  // element never reports a smaller value again, which prevented the page
  // from shrinking when switching back to "Navegar por categorias".
  const body = document.body;
  if (!body) return;
  let h = Math.ceil(body.getBoundingClientRect().height);
  // Fallback: use the bottom edge of the last visible child, in case the
  // body itself is styled to fill the viewport.
  let maxBottom = 0;
...
function scheduleHeightUpdate() {
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      sendHeight();
      // Re-measure after layout/paint has fully settled (images, fonts,
      // transitions). This guarantees we also shrink when switching back
      // to a shorter view.
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Observe only the body so we react to real content-size changes
  // (documentElement is clamped to the iframe viewport height and would
  // never fire on shrink).
